...
# 540 verilik matrisi 8 farklı değişkene göre manipüle eden simülatör motoru
def generate_advanced_sensor_data(degradation, temp_anomaly, altitude_stress, flight_mode, 
    ...
    # Tüm sürgülere göre ana hasar matrisinin taban değerini hesaplıyoruz
    ...
    # --- NOKTA ATIŞI SENSÖR MANİPÜLASYONLARI ---
    # Ana Faktörler
    simulated_data[0, :, 10] += (temp_anomaly * 0.02)  # s_11 sıcaklık sensörü
    simulated_data[0, :, 3] += (altitude_stress * 0.03) # s_4 basınç sensörü
    
    # Gelişmiş Ayarlar (Akordeon) Faktörleri
    simulated_data[0, :, 15] += (fuel_contamination * 0.04) # s_16 yakıt-hava oranı
    simulated_data[0, :, 11] += (fuel_contamination * 0.03) # s_12 yakıt akış sensörü
    simulated_data[0, :, 7] += (fan_anomaly * 0.03)         # s_8 fiziksel fan hızı sapması
    simulated_data[0, :, 14] += (bypass_degradation * 0.05) # s_15 bypass oranı
    simulated_data[0, :, 16] += (bleed_leakage * 0.04)      # s_17 tahliye entalpisi (pnömatik kaçak)
    simulated_data[0, :, 8] -= (core_fatigue * 0.03)        # s_9 çekirdek hızı (metal yorgunluğu düşüşü)
...
# --- GELİŞMİŞ AYARLAR (5 PARAMETRELİ GİZLİ AKORDEON) ---
st.sidebar.markdown("---")
...
if st.sidebar.button("🚀 Canlı Sensör Analizini Başlat", type="primary"):
    with st.spinner('Yapay Zeka Çok Boyutlu Sensör Verilerini Analiz Ediyor...'):
        ...
        # 8 Parametreyi birden simülatör motoruna gönderiyoruz
        ...
        # Geliştirilmiş RUL (Kalan Ömür) Kalibrasyon Matematiği
