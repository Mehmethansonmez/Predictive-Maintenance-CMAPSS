...
degradation = st.sidebar.slider("Yıpranma Seviyesi", 0.0, 1.0, 0.2, 0.05)
temp_anomaly = st.sidebar.slider("Sıcaklık Anomalisi", -20.0, 40.0, 0.0, 5.0)
altitude_stress = st.sidebar.slider("İrtifa / Basınç", 0.0, 5.0, 1.0, 0.5)
...
if view_mode == "📡 Canlı Telemetri Akışı":
    ...
    predicted_rul = max(0, 125 - (degradation * 85) - (active_temp * 0.5) + np.random.randint(-2, 2))
    ...
    col2.metric("Sıcaklık Sapması", f"% {int(active_temp * 0.8)}")
    st.line_chart(pd.DataFrame({"RUL": st.session_state.rul_history}), height=250)
    
    time.sleep(1.0)
    st.rerun()

# --- DERİN ANALİZ MANTIĞI ---
else:
    # Telemetriyi tamamen durdurmak için state'i temizle
    ...
    if st.button("🚀 Sensör Durumunu Analiz Et", type="primary"):
        ...
        predicted_rul = max(0, 125 - (degradation * 85) - (temp_anomaly * 0.5))
        
        st.subheader("📊 Uçuş Güvenliği Analizi")
        st.metric("Tahmini RUL", f"{int(predicted_rul)} Uçuş")
        ...
        with col_xai:
            ...
            # 4x3 Boyut ve Küçük Font
            plt.rcParams.update({'font.size': 6})
            fig, ax = plt.subplots(figsize=(4, 3))
            names = ['s_2', 's_3', 's_4', 's_7', 's_8', 's_9', 's_11', 's_12', 's_13', 's_14', 's_15', 's_17', 's_20', 's_21']
            shap.summary_plot(sv, X.reshape(-1, 14), feature_names=names, show=False, plot_size=(4.0, 3.0))
        ...
        with col_scada:
            ...
            st.json(scada)
            st.download_button("📥 İndir", json.dumps(scada), "scada.json")
