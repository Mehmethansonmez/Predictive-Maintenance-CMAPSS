...
# ==========================================
# SEKME 1: CANLI TELEMETRİ (Sekmeye tıklandığı an başlar)
# ==========================================
with tab_telemetri:
...
# ==========================================
# SEKME 2: DERİN ANALİZ (Sekmeye tıklandığında Telemetri durur)
# ==========================================
with tab_analiz:
    ...
    if st.button("🚀 Sensör Durumunu Analiz Et", type="primary"):
        with st.spinner('Yapay Zeka FD004 Verilerini Yorumluyor...'):
            ...
            with col_xai:
                ...
                # Boyut 4x3 ve küçük yazı ayarları
                plt.rcParams.update({'font.size': 6})
                fig, ax = plt.subplots(figsize=(4, 3))
                feature_names = ['s_2', 's_3', 's_4', 's_7', 's_8', 's_9', 's_11', 's_12', 's_13', 's_14', 's_15', 's_17', 's_20', 's_21']
                shap.summary_plot(sv, X_test_sample.reshape(-1, 14), feature_names=feature_names, show=False, plot_size=(4, 3))
                st.pyplot(fig, clear_figure=True, bbox_inches='tight')
                plt.close(fig)

            with col_scada:
                st.markdown("📠 **Otomasyon ve İş Emri (SCADA)**")
                scada_verisi = {"Cihaz_ID": "JET-ENG-TR-FD004", "RUL": int(predicted_rul)}
                st.json(scada_verisi)
                st.download_button("📥 İş Emri (.json)", json.dumps(scada_verisi), f"SCADA_{datetime.now().strftime('%Y%m%d')}.json")
